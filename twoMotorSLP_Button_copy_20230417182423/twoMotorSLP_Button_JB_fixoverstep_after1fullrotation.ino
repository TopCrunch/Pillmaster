...
{
  for(int i = 0; i < STEPS; i++)  // changed this to "STEPS" instead of "STEPS/3" to do the entire 360 rotation
  {
    stepBoth(increment);
  }
}
...
void loop(){
  if(digitalRead(buttonPin) == HIGH)
  {
    ...
    multiStep(1);   // now rotates both gears in a full 360 degrees
    stepMain(-1);   // main gear steps back 1 after a full rotation to remove potential overstepping that is built up
    stepAdj(1);     // adjusting gear steps forward 1 after a full rotation to remove potential overstepping that is built up
    // removed "multiStep(-1);" line since both gears are not moving backwards at the same time
